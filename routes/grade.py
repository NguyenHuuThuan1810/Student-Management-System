...
def save_grade():
    ...
    staff_id = session.get('staff_id', 'AS001') 
    ...
    try:
        ...
        if existing_grade:
            ...
            # Thêm điểm mới kèm UUID sinh ngẫu nhiên cho grade_id vì đây là kiểu VARCHAR(20)
            new_grade_id = "G" + str(uuid.uuid4().hex[:10]).upper()
            query = """
